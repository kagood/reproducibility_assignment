...
copilotStartDate <- as.Date("11/7/19", format = "%m/%d/%y") # insert the co-piloting start date in US format e.g., as.Date("01/25/18", format = "%m/%d/%y")
...
Examining the result of the t-test prior to tidying, it appears that the discrepancy is caused by the authors truncating the value "9.198" to "9.19" rather than rounding up.
```{r}
...
Again, the following discrepancy appears to stem from the authors truncating the value "4.295" to "4.29" instead of rounding up.
```{r}
...
The discrepancy here appears to be another issue of truncating rather than rounding.
```{r}
...
Copilot report:
The reproducibility report was successful. The form that the data came in was quite odd, but not too difficult to decipher. It appears that the others truncated values instead of rounding them leading to discrepancies. My only changes to the report were to comment on the source of the discrepancies.
